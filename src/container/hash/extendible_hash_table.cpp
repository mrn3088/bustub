...
void ExtendibleHashTable<K, V>::Insert(const K &key, const V &value) {
  ...
  while (true) {
    ...
    size_t global_depth = GetGlobalDepthInternal();
...
void ExtendibleHashTable<K, V>::RedistributeBucket(std::shared_ptr<Bucket> bucket) {
  ...
  num_buckets_++;
  auto p = std::make_shared<Bucket>(bucket_size_, depth);
  size_t prev_idx = std::hash<K>()(bucket->GetItems().front().first) & ((1 << (depth - 1)) - 1);
  ...
  for (size_t i = 0; i < dir_.size(); i++) {
    if ((i & ((1 << (depth - 1)) - 1)) == prev_idx && (i & ((1 << depth) - 1)) != prev_idx) {
      dir_[i] = p;
    }
  }
}
