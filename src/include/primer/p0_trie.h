 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (const auto &ch : key) {
      if (!curr->get()->HasChild(ch)) {
        curr->get()->InsertChildNode(ch, std::make_unique<TrieNode>(ch));
      }
      parent = curr;
      curr = curr->get()->GetChildNode(ch);
    }
    if (curr->get()->IsEndNode()) {
      latch_.WUnlock();
      return false;
    }
    auto new_child = std::make_unique<TrieNodeWithValue<T>>(std::move(**curr), value);
    parent->get()->RemoveChildNode(key.back());
    parent->get()->InsertChildNode(key.back(), std::move(new_child));
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    int idx = key.size() - 1;
    while (!st.empty()) {
      st.top()->get()->RemoveChildNode(key[idx]);
      st.pop();
      if (!st.empty() && (st.top()->get()->HasChildren() || st.top()->get()->IsEndNode())) {
        break;
      }
      idx--;
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    auto ptr = dynamic_cast<TrieNodeWithValue<T> *>(curr->get());
    if (ptr == nullptr) {
      latch_.RUnlock();
      return {};
    }
